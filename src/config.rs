...
/// Description written to each uploaded part.
pub fn format_part_note(group_id: &str, orig_name: &str, index: usize, total: usize, size: u64) -> String {
    format!(
        "[lanzou-part] id={group_id} name={orig_name} index={index} total={total} size={size}"
    )
}

/// Description written when a suffix was converted.
...
pub fn format_convert_note(
    ...
    format!(
        "[lanzou-convert] name={orig_name} as={upload_name} mode={mode} suffix={suffix} size={size}"
    )
}
